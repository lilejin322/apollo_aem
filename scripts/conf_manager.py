    ...
    for root, _, files in os.walk(source_dir):
        for name in files:
            ...
            if os.path.isdir(target_filename):
                logger.error("%s is a dir, please remove it manually.", target_filename)
            ...
            if os.path.islink(target_filename) and os.readlink(target_filename) == filename:
                continue
            # make sure target dir exists
            if os.path.isfile(target_dir_path):
                logger.error("%s is a file, please remove it manually.", target_dir_path)
                return False
        ...
        if args.sub_dirs:
            for d in args.sub_dirs:
                ...
                if not update(source_dir, target_dir, args.force):
                    ...
                    sys.exit(1)
        elif not update(os.path.abspath(args.source_dir), os.path.abspath(args.target_dir), args.force):
            logger.error("update conf failed!")
            sys.exit(1)
...
def _recover(args):
    ...
    if not os.path.isdir(args.recovered_dir):
        logger.error("recovered source dir [%s] not exists!", args.recovered_dir)
        sys.exit(1)

    logger.info("begin recover conf...")
    try:
        if not recover(os.path.abspath(args.target_dir), os.path.abspath(args.recovered_dir)):
...
def main():
    ...
    update_parser.add_argument('-s', '--source_dir', help='source dir path', required=True)
    update_parser.add_argument('-sub', '--sub_dirs', help='sub dir that need to be updated',
                               nargs='+', required=False),
    update_parser.add_argument('-t', '--target_dir', help='target dir path', default=APOLLO_CONF_ROOT)
    update_parser.add_argument('-f', '--force', help='force overwrite', action='store_true')
    update_parser.set_defaults(func=_update)


    recover_parser = subparsers.add_parser('recover', help='recover command help')
    recover_parser.add_argument('-t', '--target_dir', help='the dir which to be recovered', default=APOLLO_CONF_ROOT)
    recover_parser.add_argument('-r', '--recovered_dir', help='recovered source dir path', required=False)
